...
const TodoShow: React.FC<TodoShowProps> = ({ todoshow, setView }) => {
  return (
    <div className="w-full z-20 h-screen fixed top-0 left-0 bg-black/60 flex justify-center items-center">
      <div className="relative card_style p-6 rounded-lg shadow-lg w-96 flex flex-col gap-4 justify-start items-center">
        <IoClose
          onClick={() => setView(false)}
          className="absolute top-4 right-4 text-3xl  text_style cursor-pointer transition"
        />
        <h1 className="text-2xl font-semibold text_style">{todoshow.title}</h1>
        {todoshow.completed ? (
