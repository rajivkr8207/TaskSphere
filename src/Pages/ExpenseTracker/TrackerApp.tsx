...
const TrackerApp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (store) {
        ...
        setExpenses(data)
        const isadminamoount = data.filter((item)=>item.isincome).map((item)=>item.amount)
        const isadminamoountsum = isadminamoount.reduce((a, b) => a+ b, 0);
        setIncome({
            income: isadminamoountsum,
        })
        const isexpenseamoount = data.filter((item)=>item.isincome==false).map((item)=>item.amount)
  ...
  const handleaddexpense = () => {
    if (remaintrack.remain - expensetracker.amount >= 0) {
      if (expensetracker.name != "" && expensetracker.amount > 0) {
        ...
        const store = localStorage.getItem(Tracker);
        const newStore = store ? [...JSON.parse(store), expensetracker] : [expensetracker];
  ...
  return (
    <>
      <div className="w-full min-h-[88vh] bg-white  absolute top-17">
        <h1 className="text-center text-5xl font-semibold my-2 ">
          Expense Tracker{" "}
        </h1>
        <div className="flex justify-center items-center flex-col my-2">
          <h5 className="text-2xl font-semibold capitalize">your Income </h5>
        ...
        <div className="flex lg:justify-center gap-3 justify-between ">
          <div className="w-42 h-24  p-2  border-2 flex justify-center gap-3  flex-col ">
            <h5 className="text-center text-black font-medium text-lg">
              Remain balance
            </h5>
            <p
              className={` ${
                ...
                expenses?.map((item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td className="border-2 border-gray-400 p-2 capitalize">
                        {item.name}
                      </td>
                      <td
        ...
        {addexpense && (
          <div className="my-2 flex justify-center">
            <div className="w-90 p-4 bg-amber-200 relative border-2 shadow-2xl  rounded-2xl">
              <h1 className="text-3xl text-center font-semibold my-2">
                Add Expense
              </h1>
              <div
                onClick={() => setAddexpense(false)}
                className="absolute text-3xl p-1 top-5 right-5 rounded-full bg-white text-black"
              >
                <IoCloseSharp />
              </div>
              <div className="my-2">
